...
/// <summary>
/// 교전 실험용 UpperAgent.
/// 원본 UpperAgent와 동일한 obs/action 구조 (모델 호환).
/// 적을 LowerAgent 타입으로 받음 (RedLowerAgent 호환).
/// </summary>
public class BattleUpperAgent : Agent
{
    ...
    [Tooltip("적군 LowerAgents (RedLowerAgent)")]
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        for (int i = 0; i < MaxObservedEnemies; i++)
        {
            if (i < k)
            {
                ...
                sensor.AddObservation(Mathf.Clamp01(dist / 600f));
                sensor.AddObservation(1f / (dist + 1f));
